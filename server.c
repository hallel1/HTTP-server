...
void error500(serverSupport* sup,int *newsockfd);
void errorInput(char *msg);
char *get_mime_type(char *name);
int strToInteger(char *str);
int isDirectory(const char *path);
void readFromFile(serverSupport* sup,char *filename, int *newsockfd);
char* read_dir_content(serverSupport* sup,int *numbytes,char *filename,int *newsockfd,char* path,int maxlen);
...
//error 500 Treated separately (if this function used: create_conect and handle_error, the error 500 could not be performed in these functions)
	...
	int mallocLen=strlen(ERROR_500_1)+strlen(ERROR_500_2)+strlen(ERROR_500_3)+strlen(sup->timebuf)+1;
	char* msg=(char*)malloc(sizeof(char)*(mallocLen));//change
	strcpy(msg,ERROR_500_1);
	strcat(msg,sup->timebuf);
...
void handle_input(serverSupport* sup,int* newsockfd){//Disassembles the request and decides if the request is valid or need to out error msg
	...
	if(sup->flag==ERROR){
		...
		if(sup->flag==OK_AND_PRINT){
			...
			readFromFile(sup,nameFile,newsockfd);
		}
		else if(sup->flag==OK){

			msg2=read_dir_content(sup,&nbytes,nameFile,newsockfd,path,len);
			if(msg2==NULL){
				free(nameFile);
				return;
			}
			msgConect=create_conectOk(strlen(msg2),sup->timebuf,type,newsockfd,path);
...
char* read_dir_content(serverSupport* sup,int *numbytes,char *filename,int *newsockfd,char* path,int maxlen){//print the dir Items 
...
void readFromFile(serverSupport* sup,char *filename, int *newsockfd){//read from file and write to the newsockfd
